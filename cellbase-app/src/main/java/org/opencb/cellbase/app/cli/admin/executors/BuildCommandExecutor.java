...
public class BuildCommandExecutor extends CommandExecutor {
            ...
            for (String data : dataList) {
                switch (data) {
                    ...
                    case VARIATION_FUNCTIONAL_SCORE_DATA:
                        parser = buildCadd();
                        break;
                    case PROTEIN_FUNCTIONAL_PREDICTION_DATA:
                        logger.info("Protein functional prediction data (Sift and PolyPhen) is built from Perl script");
                        parser = null;
                        break;
                    case REVEL_DATA:
                        parser = buildRevel();
    ...
    private AbstractBuilder buildRevel() throws CellBaseException {
        // Sanity check
        Path revelDownloadPath = downloadFolder.resolve(PROTEIN_SUBSTITUTION_PREDICTION_DATA).resolve(REVEL_DATA);
        Path revelBuildPath = buildFolder.resolve(PROTEIN_SUBSTITUTION_PREDICTION_DATA).resolve(REVEL_DATA);
        copyVersionFiles(Collections.singletonList(revelDownloadPath.resolve(getDataVersionFilename(REVEL_DATA))), revelBuildPath);

        // Create the file serializer and the revel builder
    ...
    private AbstractBuilder buildAlphaMissense() throws CellBaseException {
        // Sanity check
        Path alphamissenseDownloadPath = downloadFolder.resolve(PROTEIN_SUBSTITUTION_PREDICTION_DATA).resolve(ALPHAMISSENSE_DATA);
        Path alphamissenseBuildPath = buildFolder.resolve(PROTEIN_SUBSTITUTION_PREDICTION_DATA).resolve(ALPHAMISSENSE_DATA);
        copyVersionFiles(Collections.singletonList(alphamissenseDownloadPath.resolve(getDataVersionFilename(ALPHAMISSENSE_DATA))),
                alphamissenseBuildPath);

        // Create the file serializer and the alphamissense builder
        CellBaseFileSerializer serializer = new CellBaseJsonFileSerializer(alphamissenseBuildPath, ALPHAMISSENSE_DATA);
        return new AlphaMissenseBuilder(alphamissenseDownloadPath, serializer);
    }
