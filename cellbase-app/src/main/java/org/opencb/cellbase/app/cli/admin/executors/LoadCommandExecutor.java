...
//>>>>>>> TASK-5564
    ...
    private List<Path> getVersionPaths(Path path) {
        ...
        if (path == null || !Files.exists(path) || !Files.isDirectory(path) || path.toFile().listFiles() == null) {
            logger.warn("Path '{}' does not exist or is not a directory or has no files. No source versions will be loaded.", path);
            return sources;
        }
