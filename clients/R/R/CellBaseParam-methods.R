...
#' (default),or GRCh38
#' @param geneId A character vector denoting the geneId/s to be queried
...
#' @examples
#' cbParam <- CellBaseParam(genome="GRCh38",geneId=c("TP73","TET1"))
#' print(cbParam)
#' @seealso for more information about the cellbase webservices see
#' \url{http://bioinfo.hpc.cam.ac.uk/cellbase/webservices/}
#' @export
CellBaseParam <- function(genome=character(), geneId=character(),
...
include=character(), exclude=character(), limit=character()){
    ...
    if(length(geneId)>0){
        ...
        geneId <- paste("geneId=",geneId,sep = "")
    }else{
        geneId <- character()
    }
    ...
    new("CellBaseParam", genome=genome, geneId=geneId, region=region, rs=rs, so=so,
