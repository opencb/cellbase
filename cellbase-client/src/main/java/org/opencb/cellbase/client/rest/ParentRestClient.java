...
public class ParentRestClient {
    ...
    protected QueryResponse<Long> count(Query query) throws IOException {
        return execute("count", query, null, Long.class);
    }

    protected <T> QueryResponse<T> execute(String action, Map<String, Object> params, QueryOptions options, Class<T> clazz) throws IOException {
        return execute(null, action, params, options, clazz);
    }

    protected <T> QueryResponse<T> execute(String id, String resource, Map<String, Object> params, QueryOptions options, Class<T> clazz)
            throws IOException {
        ...
        if (options != null) {
            for (String s : options.keySet()) {
                path = path.queryParam(s, options.get(s));
            }
        }
