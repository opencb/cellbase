...
//                }
    ...
    private void solveNegativeCodingEffect(Boolean splicing, String transcriptSequence, String chromosome,
                                           Integer transcriptStart, Integer genomicCodingStart, Integer cdnaCodingStart,
                                           Integer cdnaCodingEnd, Integer cdnaVariantStart, Integer cdnaVariantEnd,
                                           BasicDBList transcriptFlags, String variantRef, String variantAlt,
                                           HashSet<String> SoNames, ConsequenceType consequenceTypeTemplate) {
...
//                }
    ...
    private void solveStopCodonNegativeDeletion(String transcriptSequence, String chromosome, Integer transcriptStart,
                                                Integer cdnaCodingStart, Integer cdnaVariantStart,
                                                Integer cdnaVariantEnd, Set<String> SoNames) {
        Integer variantPhaseShift1 = (cdnaVariantStart - cdnaCodingStart) % 3;
        Integer variantPhaseShift2 = (cdnaVariantEnd - cdnaCodingStart) % 3;
    ...
    private void solveStopCodonNegativeInsertion(String transcriptSequence, String chromosome, Integer transcriptStart,
                                                 Integer cdnaCodingStart, Integer cdnaVariantEnd, String variantAlt,
                                                 Set<String> SoNames) {
...
//                        cdnaCodingStart -= firstCdsPhase;
    ...
    private void solveCodingNegativeTranscriptEffect(Boolean splicing, String transcriptSequence, String chromosome,
                                                     Integer transcriptStart, Integer transcriptEnd, Integer genomicCodingStart,
...
//        }
    ...
    public QueryResult getAllConsequenceTypesByVariant(GenomicVariant variant, QueryOptions options) {
        ...
        BasicDBObject transcriptInfo;
        BasicDBObject geneInfo;
        BasicDBObject regulatoryInfo;
        Integer transcriptStart, transcriptEnd;
        Integer cdsLength;
        ...
        geneInfoList = getAffectedGenesInfo(variant.getChromosome(), variantStart, variantEnd);
        ...
        List<Map> regulatoryInfoList = getAffectedRegulatoryRegionsInfo(variant.getChromosome(),
...
//        while(exonCounter<exonInfoList.size() && !splicing && variantAhead) {  // This is not a do-while since we cannot call solveJunction  until
        ...
        if(!junctionSolution[1]) {
            ...
            solveCodingNegativeTranscriptEffect(splicing, transcriptSequence, variant.getChromosome(), transcriptStart,
                    transcriptEnd, genomicCodingStart, genomicCodingEnd, variantStart, variantEnd, cdnaCodingStart,
                    cdnaCodingEnd, cdnaVariantStart, cdnaVariantEnd, cdsLength, transcriptFlags, firstCdsPhase,
                    variant.getReference(), variant.getAlternative(), SoNames,
                    consequenceTypeTemplate);
        }
    }
...
//        while(exonCounter<exonInfoList.size() && !splicing && variantAhead) {  // This is not a do-while since we cannot call solveJunction  until
    ...
    public List<QueryResult> getAllConsequenceTypesByVariantList(List<GenomicVariant> variants, QueryOptions options) {
        ...
        for (GenomicVariant genomicVariant : variants) {
            ...
            int variantEnd = genomicVariant.getPosition() + genomicVariant.getReference().length() - 1;  //TODO: Check deletion input format to ensure that variantEnd is correctly calculated
            Boolean isInsertion = genomicVariant.getReference().equals("-");
            int variantStart;
            if(isInsertion) {
                variantStart = genomicVariant.getPosition()-1;
            } else {
                variantStart = genomicVariant.getPosition();
            }

            // Execute query and calculate time
            long dbTimeStart = System.currentTimeMillis();
            QueryResult queryResult = getAllConsequenceTypesByVariant(genomicVariant, options);
    ...
    private List<Map> getAffectedGenesInfo(String chromosome, int variantStart, int variantEnd) {
        ...
        QueryResult geneQueryResult = geneDBAdaptor.getAllByRegion(new Region(chromosome, variantStart-5000,
                variantEnd+5000), geneQueryOptions);
        return (LinkedList) geneQueryResult.getResult();
    }

    private List<Map> getAffectedRegulatoryRegionsInfo(String chromosome, int variantStart, int variantEnd) {
        QueryResult regulatoryQueryResult = regulatoryRegionDBAdaptor.getAllByRegion(new Region(chromosome, variantStart,
                variantEnd), new QueryOptions());
        return (LinkedList) regulatoryQueryResult.getResult();
    }
...
//                queryResult.setResult(document);
    ...
    public List<QueryResult> getAnnotationByVariantList(List<GenomicVariant> variantList, QueryOptions queryOptions) {
        ...
        if (includeList.isEmpty()) {
            includeList = Arrays.asList("variation", "clinical", "consequenceType", "conservation", "drugInteraction");
        }
        ...
        List<QueryResult> drugInteractionList = null;
        if (includeList.contains("drugInteraction")) {
            drugInteractionList = getAlldrugInteractions(variantList, queryOptions);
            if (annotations == null) {
                annotations = variationConsequenceTypeList;
            }
        }
